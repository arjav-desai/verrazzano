...
build: manifests generate go-mod
	go build -o bin/platform-operator operator/main.go
...
.PHONY: go-fmt
go-fmt:
	gofmt -s -e -d $(shell find . -name "*.go" | grep -v /vendor/ | grep -v /operator/pkg/assets/) > error.txt
	if [ -s error.txt ]; then\
		cat error.txt;\
		rm error.txt;\
		exit 1;\
...
go-ineffassign:
	...
	ineffassign $(shell find . -name "*.go" | grep -v /vendor/ | grep -v /operator/pkg/assets/)
