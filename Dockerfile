...
# Clone the verrazzano repository to obtain the installation scripts
# NOTE: Temporarily working off the shared branch that contains the external config file changes
WORKDIR /
RUN git clone https://github.com/verrazzano/verrazzano.git \
    && cd verrazzano \
    && git checkout shared/VZ-1607
...
RUN yum update -y \
    ...
    && yum install -y binutils-2.27-44.base.0.1.el7.x86_64 \
